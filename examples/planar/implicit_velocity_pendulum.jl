...
"""Eight implicit equations for the deficit-one velocity-constraint model."""
function implicit_velocity_pendulum!(equations, dy, y,
    ...
    equations[7:8] .= y[4:5] .+ d_global .* y[6]
    return nothing
end

"""Analytical matrix F_y + coefficient*F_dy for the deficit-one model."""
function implicit_velocity_jacobian(t, y, dy, coefficient,
    ...
    # d(V + d*omega)/dtheta = -r*omega.
    jacobian[7:8, 3] .= -r_global .* y[6]
    jacobian[7, 4] = 1
    jacobian[8, 5] = 1
...
function implicit_velocity_jacobian!(jacobian, dy, y,
                                     p::PendulumParameters, coefficient, t)
    jacobian .= implicit_velocity_jacobian(t, y, dy, coefficient, p)
    return nothing
end

function historical_velocity_residual!(equations, t, y, dy,
                                        p::PendulumParameters)
    implicit_velocity_pendulum!(equations, dy, y, p, t)
    return nothing
end


function historical_velocity_jacobian!(jacobian, t, y, dy, coefficient,
                                        p::PendulumParameters)
    jacobian .= implicit_velocity_jacobian(t, y, dy, coefficient, p)
    return nothing
end

function implicit_velocity_problem(; theta0 = deg2rad(45.0),
                                     omega0 = 0.0,
                                     tspan = (0.0, 5.0),
    ...
    implicit_function = DAEFunction(
        implicit_velocity_pendulum!; jac = implicit_velocity_jacobian!
    )
    return DAEProblem(
...
function velocity_control_tolerances(;
        controlled = 1.0e-9, suppressed = 1.0e6)
    return [fill(controlled, 6); fill(suppressed, 2)]
end

function run_velocity_dassl(; theta0 = deg2rad(45.0),
                              omega0 = 0.0,
                              tspan = (0.0, 5.0),
    ...
    abstol = velocity_control_tolerances(
    ...
    jacobian = (t, y, dy, coefficient) ->
        implicit_velocity_jacobian(t, y, dy, coefficient, p)
...
function run_velocity_ida(; theta0 = deg2rad(45.0),
                            omega0 = 0.0,
                            tspan = (0.0, 5.0),
                            controlled_abstol = 1.0e-9,
                            suppressed_abstol = 1.0e6,
                            reltol = 1.0e-8)
    problem = implicit_velocity_problem(; theta0, omega0, tspan)
    abstol = velocity_control_tolerances(
        controlled = controlled_abstol,
        suppressed = suppressed_abstol,
    )
...
"""Run the deficit-one pendulum with the Julia DDASSL reconstruction."""
function run_velocity_historical_ddassl(;
        theta0 = deg2rad(45.0),
        omega0 = 0.0,
        tspan = (0.0, 5.0),
        atol = 1.0e-7,
    ...
    return HistoricalDDASSL.dassl(
        historical_velocity_residual!, y0, dy0, tspan;
        parameter = p,
        jacobian! = historical_velocity_jacobian!,
        options,
        variable_levels,
        equation_levels,
        differential_vars,
...
function velocity_solution_diagnostics(solution, p::PendulumParameters)
    ...
    for (y, dy, t) in zip(solution.u, solution.du, solution.t)
        ...
        push!(velocity_errors, norm(equations[7:8], Inf))
        push!(
            acceleration_errors,
...
function historical_velocity_solver_diagnostics(solution)
...
if abspath(PROGRAM_FILE) == @__FILE__
    p = PendulumParameters()
    for (name, runner) in (
        ("Julia DDASSL reconstruction", run_velocity_historical_ddassl),
        ("Native SciML DASSL", run_velocity_dassl),
        ("Sundials IDA", run_velocity_ida),
    )
        println(name)
        try
            solution = runner()
            diagnostics = velocity_solution_diagnostics(solution, p)
            ...
            if solution isa HistoricalDDASSL.DASSLResult
                for (key, value) in pairs(
                        historical_velocity_solver_diagnostics(solution))
                    println("  ", key, ": ", value)
                end
            end
        catch error
